...
interface ReminderModalProps {
  appt: MedicalAppointmentResponse;
  onClose: () => void;
  onEdit?: (appt: MedicalAppointmentResponse) => void;
...
function ReminderModal({ appt, onClose, onEdit, formatTimeRange }: ReminderModalProps) {
  ...
  const handleSendReminder = async () => {
    ...
    const payload = {
      ...
      cita: {
        id: appt.id,
        fecha: appt.appointmentDate?.split("T")[0] ?? "",
        ...
        <div className="space-y-3.5 py-2 text-xs">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="p-2.5 border rounded-lg bg-background">
              ...
              <div className="font-bold text-red-600 mt-0.5">
                {appt.appointmentDate?.split("T")[0] || "Sin fecha"}
              </div>
            </div>
            <div className="p-2.5 border rounded-lg bg-background">
              <span className="text-[10px] font-semibold text-muted-foreground flex items-center gap-1">
          ...
          <div className="p-2.5 border rounded-lg bg-background flex items-center justify-between">
            ...
            <span className="font-medium text-foreground">
              {appt.nextAppointmentDate?.split("T")[0] || "No programada"}
            </span>
          </div>
          ...
          <div className="flex items-center justify-between p-2.5 border rounded-lg bg-background">
            <span className="text-xs font-semibold text-muted-foreground flex items-center gap-1.5">
              <Bot className="size-4 text-emerald-600" /> Canal de Origen
            </span>
            <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-muted text-foreground">
              {appt.contact?.type === "WHATSAPP" || (appt.contact?.type as string) === "BOT"
                ? "Bot de WhatsApp"
                : "Registro Manual"}
            </span>
          </div>
...
export function AppointmentCalendar({
  ...
  const [selectedAppt, setSelectedAppt] = useState<MedicalAppointmentResponse | null>(null);
  ...
  const appointmentsByDate = appointments.reduce<Record<string, MedicalAppointmentResponse[]>>(
    (acc, appt) => {
                  ...
                  {dayAppts.map((appt) => {
                    ...
                    return (
                      <div
                        ...
                        className={`w-full text-left p-1.5 rounded-md text-[11px] border transition-all shadow-2xs flex flex-col gap-0.5 cursor-pointer group relative ${
                          isBot
                      ...
                      >
                        <div className="font-semibold truncate flex items-center justify-between gap-1">
                          <span className="truncate flex items-center gap-1">
                            {isBot && <Bot className="size-3 text-emerald-600 shrink-0" />}
                            {appt.patientFullName || "Paciente N/A"}
                          </span>
