...
import { alertsApi, type AlertEvent, type CreateAlertEventInput } from "@/api/alerts";
...
export function useAddAlertEvent(alertId: string | null) {
  ...
  return useMutation({
    mutationFn: (data: CreateAlertEventInput) =>
      alertId ? alertsApi.addEvent(alertId, data) : Promise.reject(new Error("No alert ID")),
    onSuccess: () => {
...
export function useGenerateAISummary(alertId: string | null) {
  ...
  return useMutation({
    mutationFn: () =>
      alertId ? alertsApi.generateAISummary(alertId) : Promise.reject(new Error("No alert ID")),
    onSuccess: () => {
