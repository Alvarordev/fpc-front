...
export const patientColumns: ColumnDef<PatientListItem>[] = [
  ...
  { id: "status", header: "Estado", cell: ({ row }) => <div className="flex gap-1"><Badge className={cn("border", row.original.status === "ENROLLED" ? "bg-blue-50 text-blue-700" : "bg-violet-50 text-violet-700")}>{row.original.status === "ENROLLED" ? "Enrolado" : "Sin enrolar"}</Badge><Badge className={cn("border", row.original.isActive ? "bg-emerald-50 text-emerald-700" : "bg-zinc-100 text-zinc-600")}>{row.original.isActive ? "Activo" : "Inactivo"}</Badge></div> },
]
