  ...
  it("enrolls the diagnosis branch through the canonical transaction", async () => {
    ...
    const enrollment = await request<Enrollment>("/enrollments", {
      ...
      body: JSON.stringify({
        ...
        details: { currentDepartment: "Lima", referredToSocialWorker: true },
        insurance: { insuranceType: "EPS", epsProvider: "RIMAC" },
        diagnosis: { diagnosis: "Cáncer de mama", healthCenterId: healthCenter?.id },
        treatment: { treatmentType: "Quimioterapia", healthCenterId: healthCenter?.id },
        symptomReport: { hasDiscomfort: true, signsAndSymptoms: "Dolor persistente", indicationsReceived: "Control" },
