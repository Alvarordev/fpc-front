...
runDescribe("enrollment flow against Nest", () => {
  it("enrolls the diagnosis branch through the canonical transaction", async () => {
    ...
    const enrollment = await request<Enrollment>(
      "/enrollments",
      {
        method: "POST",
        body: JSON.stringify({
          patient: {
            fullName: "Test Enrolamiento Nest",
    ...
    const addresses = await request<PatientAddress[]>(
      `/patients/${enrollment.patientId}/addresses`,
      {},
      token,
    )
