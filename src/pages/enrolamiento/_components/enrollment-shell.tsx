...
function CurrentStep({
  ...
  switch (step) {
    ...
    case 2:
      return <Step2Consent embedded={embedded} />
    case 3:
      return <Step3Identificacion embedded={embedded} />
    case 4:
      return <Step4Consentimiento embedded={embedded} />
    case 5:
      return <Step5Datos embedded={embedded} historical={historical} />
    case 6:
      return <Step6Categoria embedded={embedded} />
    case 7:
      return <Step7Atencion embedded={embedded} historical={historical} />
    case 8:
      return <Step8Cierre historical={historical} embedded={embedded} />
    default:
      return (
        <Step1Inicio
          onOpenNotes={onOpenNotes}
...
export function EnrollmentShell({
    ...
    <div className="mx-auto max-w-3xl space-y-12 px-5 py-8 md:px-8 md:py-10">
      {Array.from({ length: TOTAL_STEPS }, (_, index) => index + 1).map(
        (step) => (
          <section
            key={step}
            className="border-border/50 border-b pb-12 last:border-b-0 last:pb-0"
          >
            <CurrentStep
              step={step}
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      {!continuous && (
        <div className="border-border/50 bg-background flex shrink-0 items-center justify-center border-b px-6 py-4">
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        {!continuous && (
          <div className="border-border/50 bg-muted/30 hidden w-80 shrink-0 overflow-y-auto border-l lg:block xl:w-96">
      ...
      <div className="pointer-events-none fixed right-5 bottom-5 z-40 lg:hidden">
        <Button
          type="button"
          size="lg"
        ...
        >
          ...
          <span>Notas</span>
          {notesCount > 0 && (
            <span className="bg-primary-foreground/15 flex size-5 items-center justify-center rounded-full text-xs">
              {notesCount}
            </span>
          )}
        </Button>
      </div>
      <EnrollmentNotesSheet
        key={notesResetKey}
        open={notesOpen}
