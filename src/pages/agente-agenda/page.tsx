...
function FollowUpPanel({ title, icon: Icon, items, navigate }: { title: string; icon: typeof Phone; items: Awaited<ReturnType<typeof followUpsApi.list>>; navigate: ReturnType<typeof useNavigate> }) {
 return <Card className="overflow-hidden"><CardHeader className="border-b bg-muted/20"><div className="flex items-center justify-between"><CardTitle className="flex items-center gap-2 text-base"><span className="flex size-8 items-center justify-center rounded-full bg-amber-50"><Icon className="size-4 text-amber-600" /></span>{title}</CardTitle><Badge variant="secondary">{items.length}</Badge></div></CardHeader><CardContent className="p-0">{items.length === 0 ? <div className="flex flex-col items-center justify-center gap-2 py-12 text-muted-foreground"><PhoneCall className="size-8 opacity-30" /><p className="text-sm">No hay seguimientos para mostrar</p></div> : <div className="divide-y">{items.sort((a, b) => (a.scheduledAt ?? "").localeCompare(b.scheduledAt ?? "")).map((followUp) => <PatientRow key={followUp.id} patientName={followUp.subjectPatientName ?? "Paciente desconocido"} icon={Phone} title={typeLabels[followUp.type]} meta={`${purposeLabels[followUp.purpose]} · ${followUp.scheduledAt ? `${formatDate(followUp.scheduledAt)} ${formatTime(followUp.scheduledAt)}` : "Sin fecha"}`} onClick={() => navigate(`/pacientes/${followUp.subjectPatientId}/seguimientos/${followUp.id}`)} />)}</div>}</CardContent></Card>
}
